...
router.post('/register', async (req, res) => {
    try {
        const { firstname, email, lastname, password } = req.body; //object destructure
        if (!firstname || !email || !lastname || !password) {
            return res.status(400).send({ message: 'All fields are mandatory11111111' })

        }
        const newUser = new usermodel({ firstname, email, lastname, password });
